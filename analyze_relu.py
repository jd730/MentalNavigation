"""Driver for the Vector-HaSH ReLU-variant sweep analyses.

Runs the per-paper analysis suite on the ``decoder_dir_mental/ReLU/``
sweep (50 seeds of VHA-ReLU + 50 seeds of VHA-ReLU-D), reusing existing
analysis modules:
...
Usage:
    python analyze_relu.py                     # everything, seeds 0..49
    python analyze_relu.py -focus_seed 43      # single-neuron analyses on 43
    python analyze_relu.py -skip rdm speed     # skip RDM + speed blocks
"""
import argparse
import glob
import os
...
SWEEP_DIR = 'decoder_dir_mental/ReLU'
SAVE_DIR = 'results/ReLU'
...
LEGACY_BASELINE_DIRS = {
    ...
    'RNN-D': 'decoder_dir_mental/REV_ABS/vRNN2',      # double CTRNN, no grid cells (50 seeds)
}
BASELINE_MODELS = list(LEGACY_BASELINE_DIRS.keys())
RELU_MODELS = ['VHA-ReLU', 'VHA-ReLU-D']
MODELS = RELU_MODELS  # kept for periodicity/RDM code that iterates ReLU only.
ALL_MODELS = BASELINE_MODELS + RELU_MODELS
...
def _model_from_name(run_name):
    """Return 'VHA-ReLU-D' or 'VHA-ReLU' from the run-directory name."""
    return 'VHA-ReLU-D' if run_name.startswith('VHA-ReLU-D_') else 'VHA-ReLU'


def _model_constraint(model):
    """Substring that matches this model but not the other."""
    return 'VHA-ReLU-D_' if model == 'VHA-ReLU-D' else 'VHA-ReLU_'
...
def load_all_logs(dirname, seeds=None):
    ...
    """
    dfs = []
    # ReLU models from the current sweep dir.
...
MODEL_COLORS = {
    ...
    'VHA-ReLU':   '#33669a',   # blue
    'VHA-ReLU-D': '#d95527',   # orange
}
MODEL_LINESTYLES = {
    'RNN':        '--',
    'RNN-D':      '--',
...
def aggregate_accuracy(dfs, save_dir):
    ...
    # ---- Bar + box plots per model (env-0 speed, seen/unseen aggregated) ----
    speed_long = (speed_wide.replace(np.inf, np.nan)
                            .reset_index()
                            .melt(id_vars='seed', var_name='model',
                                  value_name='trial_to_thresh')
                            .dropna())
    order = [m for m in ALL_MODELS if m in speed_long['model'].unique()]
    ylabel = f'Trials to reach {int(THRESHOLD*100)}% accuracy (visual/all/env0)'
    fig, ax = plt.subplots(figsize=(6, 4), dpi=200)
    sns.barplot(data=speed_long, x='model', y='trial_to_thresh', order=order,
                errorbar='se', capsize=0.2,
                palette=[MODEL_COLORS.get(m) for m in order], ax=ax)
    ax.set_ylabel(ylabel)
    ax.set_xlabel('')
    plt.xticks(rotation=30, ha='right')
    plt.tight_layout()
    plt.savefig(os.path.join(save_dir, 'speed_bar.pdf'))
    plt.close(fig)

    # Box plot: VHA-ReLU vs VHA-ReLU-D only (drop RNN baselines); boxes in
    # neutral grey since this panel is about the two ReLU variants, not the
    # full baseline comparison.
    box_order = [m for m in RELU_MODELS if m in speed_long['model'].unique()]
...
_BAR_KEEP_NAMES = [
    'non_zeros_box.pdf', 'periodicity_at_LM_box.pdf', 'ratio_box.pdf',
]
_BAR_DROP_NAMES = [
...
def parse_args():
    ...
    p.add_argument('-dirname', default=SWEEP_DIR,
                   help='Root sweep directory for ReLU variants '
                        '(default: %(default)s). RNN and RNN-D baselines '
                        'are read from the legacy dirs in '
                        'LEGACY_BASELINE_DIRS at the top of this module.')
