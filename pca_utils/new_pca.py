...
#    paths = glob.glob('norm25_skipmult/random*S5*.mat')
    ...
    paths = glob.glob('REV/random*S*.mat') # 128
    paths = glob.glob('REV_ABS/random*S*.mat') # 128
    paths = glob.glob('REV/random*S5*.mat') # 128
    N = 220
    N = 200
    N = 50
    N = 32
...
#    for sigma in [50, 100, 200, 400, 800]:
        ...
        with open(f'monkey_neural_embeddings_min_{int(sigma)}_radius_{radius}.pkl', 'wb') as f:
...
#        visualize_grid(_embeddings, titles=titles, prefix=f'monkey{int(sigma)}_radius_{radius}')
            


#            visualize(hidden_embedding, title=f'{mode} of {name}')
    if True:
        validN = 0 
        ...
        for path in sorted(paths):
            ...
            seed = path.split('/')[-1].split('Ep')[0].split('S')[-1]
    ...
    #                _logits[k] = gaussian_filter1d(v, sigma=4, axis=2, truncate=2)
                ...
                for name, hidden in zip(['distRNN', 'actionRNN'], [_base_hidden, _hidden]):
                    for mode in ['pca']: #, 'isomap']: #, 'tsne']:
                        hidden_embedding = nonlinear_analysis(hidden, distance, mode)
                        embeddings[f'{seed}{mode}_{name}_{direction}'] = hidden_embedding
    #                embeddings[name] = (hidden, distance)
                ...
                embeddings[f'{seed}{mode}_logits_{direction}'] = logit_embedding
                embeddings[f'{seed}logits_{direction}'] = (_logits, distance)
                embeddings[f'{seed}softmax_{direction}'] = (softmax_logits, distance)
#                embeddings[f'{seed}softmax_{direction}_PCA'] = softmax_logits
                print(direction)
#            with open(f'monkey_neural_embeddings_CONT_RNN.pkl', 'wb') as f:
#                pickle.dump(embeddings, f)
...
#    hidden_embedding = nonlinear_analysis(hidden, distance, mode)
#    base_hidden_embedding = nonlinear_analysis(base_hidden, distance, mode)


#    # Visualizations
#    visualize(base_hidden_embedding, title=f'{mode} of Dist RNN')
#    visualize(hidden_embedding, title=f'{mode} of Action RNN')
